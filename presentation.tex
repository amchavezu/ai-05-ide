...
\newcommand{\sourcefoot}{\scriptsize\color{Muted}\textbf{PAPER.} Ide and Talam\`as (2025), accepted manuscript v12.}
\newcommand{\derivationfoot}{\scriptsize\color{Muted}\textbf{DERIVATION.} Author's handwritten reconstruction of the discrete mechanism.}
\newcommand{\interpretationfoot}{\scriptsize\color{Muted}\textbf{INTERPRETATION.} Economic reading of Propositions 5 and 6.}
...
\begin{frame}{Why AI changes more than productivity}
\small
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.47\textwidth}
  \key{Before AI}

  Many workers handle routine cases. Scarce experts answer the difficult
  questions that workers cannot solve.

  \vspace{0.24cm}
  A hierarchy lets one expert's knowledge support many workers, but expert
  time remains scarce.
\end{column}
\begin{column}{0.49\textwidth}
  \key{The AI shock}

  AI adds scalable knowledge. Its effect depends on how difficult a problem it
  can solve and whether it may produce on its own.

  \vspace{0.24cm}
  Those two features change who works with whom and how the team divides its
  output.
\end{column}
\end{columns}
...
\textbf{The paper's question.} When AI enters the hierarchy, who gets a better
expert, who captures the surplus, and does every unit of compute produce?

\vspace{0.08cm}
\sourcefoot
\end{frame}

\begin{frame}{Capability versus autonomy}
\small
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.47\textwidth}
...
\begin{frame}{Model objects and the agent's problem}
\small
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.47\textwidth}
  \key{Primitives}

  \textbf{$z\in[0,1]$} \; human knowledge: fraction solved alone.\\[0.10cm]
  \textbf{$x\sim U[0,1]$} \; problem difficulty.\\[0.10cm]
  \textbf{$h\in(0,1)$} \; solver time consumed by one question.\\[0.10cm]
  \textbf{$a\equiv z_{AI}$} \; AI capability.\\[0.10cm]
  \textbf{$\mu$} \; available units of compute.
\end{column}
\begin{column}{0.49\textwidth}
  \key{Equilibrium objects}

  \textbf{$n(z)$} \; workers supervised by one solver.\\[0.10cm]
  \textbf{$w(z),\ r$} \; human wage and compute rental price.\\[0.10cm]
  \textbf{$W,I,S$} \; human workers, independent producers, and solvers.\\[0.10cm]
  \textbf{$m(z)$} \; solver knowledge matched to worker $z$.\\[0.10cm]
  \textbf{$W_a,S_a$} \; humans paired with AI on either layer.
\end{column}
\end{columns}
...
Firms choose the organizational form, roles, and matches that maximize
\[
\Pi=\mathbb E[\text{output}]-\text{human wages}-\text{compute cost},
\]
subject to scarce solver time and market clearing. Humans choose the
highest-paying occupation; compute owners rent to the highest bidder.

\vspace{0.06cm}
\sourcefoot
\end{frame}

\begin{frame}{One expert can support many workers}
\small
\begin{columns}[T,totalwidth=\textwidth]
...
\begin{frame}{A worker's wage reflects the match and the split}
\small
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.54\textwidth}
...
\begin{frame}{Autonomy gives AI a productive outside option}
\small
...
\begin{frame}{Capability decides where AI enters the hierarchy}
\small
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.48\textwidth}
...
\begin{frame}{Proposition 5: capability decides whether the bottom gains}
\small
...
\begin{frame}{A simple threshold explains the two numerical cases}
\small
...
\begin{frame}{Proposition 6: restricting autonomy redistributes gains}
\small
...
\begin{frame}{Nonautonomy raises bottom pay but sacrifices output}
\small
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.49\textwidth}
...
\begin{frame}{How to read the paper's results}
\small
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.47\textwidth}
  \key{First ask: how capable is AI?}

  If capability is too low, firms may not use a nonautonomous AI at all.

  \vspace{0.22cm}
  Within the autonomous regime, capability determines whether the least
  knowledgeable workers gain. The bottom needs AI to cross an endogenous
  threshold.
\end{column}
\begin{column}{0.49\textwidth}
  \key{Then ask: what may AI do?}

  An autonomous AI can produce alone. That outside option raises the compute
  price and keeps spare compute productive.

  \vspace{0.22cm}
  A nonautonomous AI transfers more surplus to assisted workers, but leaves
  some compute idle and reduces total output.
\end{column}
\end{columns}

\vfill
{\color{Coral}\rule{\textwidth}{0.8pt}}\\[-0.02cm]
\textbf{Bottom line.} A restriction on autonomy can help some low-knowledge
workers while making the economy produce less. Capability and autonomy answer
different parts of the distributional question.
...
\scriptsize
  \key{1. Paper equation and discrete claim}
  \[
  w^A(z)=a[1-h(1-z)]
  \quad\Longrightarrow\quad
  w^A(0)>w_0
...
\begin{frame}[plain]
\centering
\includegraphics[width=\textwidth,height=0.96\paperheight,keepaspectratio,
  clip,trim=0 800bp 0 0]{hand/derivation-threshold.jpg}
\end{frame}

\begin{frame}[plain]
\centering
\includegraphics[width=\textwidth,height=0.96\paperheight,keepaspectratio,
  clip,trim=0 0 0 720bp]{hand/derivation-threshold.jpg}
\end{frame}

\begin{frame}[plain]
\centering
\includegraphics[width=\textwidth,height=0.96\paperheight,keepaspectratio]
  {hand/derivation-autonomy.jpg}
\end{frame}
